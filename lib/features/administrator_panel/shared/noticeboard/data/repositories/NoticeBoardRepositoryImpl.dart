...
class NoticeBoardRepositoryImpl implements NoticeBoardRepository {
  final NoticeBoardRemoteDataSource noticeRemoteDataSource;
  NoticeBoardRepositoryImpl({required this.noticeRemoteDataSource});

  @override
  Future<Either<Fail, MainNotice>> addNotice(MainNotice notice) async {
    try {
      return Right(await noticeRemoteDataSource
          .addNotice(NoticeModel.fromNotice(notice)));
    } catch (e) {
  ...
  Future<Either<Fail, void>> deleteNotice(MainNotice notice) async {
    try {
      return Right(await noticeRemoteDataSource.deleteNotice(notice.id));
    } catch (e) {
      String message = e.toString();
      int startIndex = message.indexOf(']');
      if (startIndex != -1) {
  ...
  Future<Either<Fail, MainNotice>> editNotice(MainNotice notice) async {
    try {
      return Right(await noticeRemoteDataSource
          .editNotice(NoticeModel.fromNotice(notice)));
    } catch (e) {
      String message = e.toString();
      int startIndex = message.indexOf(']');
      if (startIndex != -1) {
